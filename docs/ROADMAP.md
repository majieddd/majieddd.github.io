...
| # | Note | Status |
|---|---|---|
| 19.5 | **Robotic towers unlock through the story only** — beat a solar system to earn one. **DRONE BAY is the first.** | ✅ ladder DRONE BAY → RAILGUN → ECHO → PYLON → FOUNDRY → QUAKE → NULLFIELD → SINGULARITY → VAULT → REPLICATOR (ten steps since 19.21 took robotic to ten); `unlockTower` and `canUnlockTower` both refuse them, shop lists them as story unlocks with systems owed |
...
| 19.22 | **Distinct balance curves.** Some towers early-game, some late — via price, difficulty, or a mechanic that needs time on the field. Unique, but fair. | ✅ each of the eleven states its shape — opener / conditional / investment — and expresses it as a mechanic rather than a price. Pins measured at the time (unseeded): loss median 6 over n=12. **Session 21 correction:** the seeded re-baseline supersedes those numbers — fresh loss median 8, and “every loss under wave 10” is false (coil 10, shattered 13, reproducibly). See [`BALANCE-BASELINE.md`](BALANCE-BASELINE.md) |
...
| # | Note | Status |
|---|---|---|
| 19.23 | **≥5 boons per faction** (20+ total), each reading as a reflection of the planet it came from. | ✅ **20 boons, 5 per power**, written out of each power's lore. All 20 move an observable engine value — but that claim was FALSE when first written and stood for two sessions: the check behind it snapshotted `traits` only, so a boon writing a trait nothing reads counted as live. AUREOLE and LETTERS OF MARQUE were wholly inert and SALVAGE RIGHTS and SCUTTLE half inert, because BOON_FOLD wrote four keys whose only route to the engine is `foldTraits` — which runs 246 lines EARLIER in Game.start. Fixed in Session 22; the check now watches `mods` as well, which is what makes the claim testable at all. Wakes **five dormant mechanics** (`freeCopies`, `lastStandAt`/`Dmg`, `waveHeal`, `immortalLine`, `siphonRate`) whose engine code was finished but which nothing ever wrote |
