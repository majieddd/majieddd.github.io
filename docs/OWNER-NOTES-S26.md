...
| 4 | De-bloat descriptions everywhere: mechanics first, flavor brief + italic below; never the em dash | shipped (269 string literals cleaned by a lexer-driven pass with deterministic punctuation rules, plus 9 &mdash; entities and every visible index.html line; measured zero em dashes rendered on all four major screens. copyHTML renderer: stats lead, desc follows, lore renders italic below; worst-10 towers rewritten mechanics-first with lore fields, PRIVATEER and MONSTRANCE corrected against the actual code while rewriting) |
