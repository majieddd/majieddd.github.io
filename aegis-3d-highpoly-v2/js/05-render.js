...
var R = (function () {
  ...
  function init(cv, opts) {
    ...
    /* Quality hook: read or set a live quality knob (bloom, shadows, ink,
       shadowSize, scale). Used by the perf audit to A/B passes in place. */
    window.__RQUAL = function (key, val) {
      if (key === undefined) return { shadowSize: quality.shadowSize, scale: quality.scale, bloom: quality.bloom, shadows: quality.shadows, ink: quality.ink };
      if (val !== undefined) quality[key] = val;
      return quality[key];
    };

    toothTex = GL.texFromCanvas(PAINT.buildAtlas(1024));
    ...
    setPalette('human');
    /* Exposed so the manual-frame hook can force raster completion (perf). */
    R._glFinish = function () { try { gl.finish(); } catch (e) {} };
    return gl;
  }
