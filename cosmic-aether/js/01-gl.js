...
var GL = (function () {
  ...
  /* ---------- multisampled target ----------
     The renderer's HD path renders the scene into a multisampled framebuffer
     and resolves it to a plain texture pair with blitFramebuffer. That is the
     canonical WebGL2 way to get true spatial antialiasing while the pipeline
     still runs its post passes on textures (ink, bloom, composite). The
     alternative, FXAA on the resolved image, blurs detail; the alternative,
     no AA at all, leaves the cel and neon edges stair-stepped, which is the
     loudest "this is a browser demo" signal a picture can carry.

     HDR note: the colour renderbuffers keep the same RGBA16F internal format
     the textured path uses (gated by EXT_color_buffer_float), so bloom still
     has a place to see values above 1.0. */
  function targetMSAA(w, h, samples, spec) {
    spec = spec || {};
    var colorSpecs = spec.color || [{ internal: null, format: null, type: null }];
    var fb = gl.createFramebuffer();
    gl.bindFramebuffer(gl.FRAMEBUFFER, fb);
    ...
    var hdr = caps.colorBufferFloat;
    var n = Math.min(samples, caps.maxSamples);
    var rbs = [];
    var bufs = [];
    for (var i = 0; i < colorSpecs.length; i++) {
      var cs = colorSpecs[i];
      ...
      gl.renderbufferStorageMultisample(gl.RENDERBUFFER, n, internal, w, h);
    ...
    if (status !== gl.FRAMEBUFFER_COMPLETE) {
      ...
      names[gl.FRAMEBUFFER_UNSUPPORTED] = 'UNSUPPORTED';
      names[gl.FRAMEBUFFER_INCOMPLETE_MULTISAMPLE] = 'INCOMPLETE_MULTISAMPLE';
      fail('framebuffer', (names[status] || status) + ' (msaa ' + n + ') at ' + w + 'x' + h);
    }
    gl.bindFramebuffer(gl.FRAMEBUFFER, null);

    return {
      fb: fb, rbs: rbs, depthRb: depthRb, w: w, h: h, samples: n,
      bind: function () {
        gl.bindFramebuffer(gl.FRAMEBUFFER, fb);
        gl.viewport(0, 0, w, h);
      },
      dispose: function () {
        gl.deleteFramebuffer(fb);
        for (var i = 0; i < rbs.length; i++) gl.deleteRenderbuffer(rbs[i]);
        if (depthRb) gl.deleteRenderbuffer(depthRb);
      }
    };
  }

  /* Resolve one colour attachment of a multisample read framebuffer into the
     corresponding attachment of a texture target.

     THE DRIVER-FACING DETAIL: blitFramebuffer honours DRAW_BUFFER[0] only.
     An earlier version set drawBuffers([COLOR_ATTACHMENT0 + index]) and
     skipped around, but a probe on D3D11 ANGLE measured the second blit
     writing nothing at all: the blit target is the first draw buffer, period.
     The portable shape is therefore to DETACH every colour attachment from
     the draw framebuffer, attach the one destination texture at
     COLOR_ATTACHMENT0, blit, and restore. One attachment, one draw buffer,
     every driver agrees. */
  function blitResolve(srcMS, dst, index, filter) {
    var i;
    gl.bindFramebuffer(gl.READ_FRAMEBUFFER, srcMS.fb);
    gl.readBuffer(gl.COLOR_ATTACHMENT0 + (index || 0));
    gl.bindFramebuffer(gl.DRAW_FRAMEBUFFER, dst.fb);
    for (i = 0; i < dst.textures.length; i++) {
      gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0 + i, gl.TEXTURE_2D, null, 0);
    }
    gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, dst.textures[index || 0], 0);
    gl.drawBuffers([gl.COLOR_ATTACHMENT0]);
    gl.blitFramebuffer(0, 0, srcMS.w, srcMS.h, 0, 0, dst.w, dst.h,
      gl.COLOR_BUFFER_BIT, filter || gl.NEAREST);
    /* Restore the two-attachment layout the pipeline expects. */
    for (i = 0; i < dst.textures.length; i++) {
      gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0 + i, gl.TEXTURE_2D, dst.textures[i], 0);
    }
  ...
  return {
    ...
    target: target, targetMSAA: targetMSAA, blitResolve: blitResolve,
    shadowTarget: shadowTarget, bindScreen: bindScreen,
    errors: errors,
