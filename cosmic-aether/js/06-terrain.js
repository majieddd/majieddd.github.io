...
var TERRAIN = (function () {
  ...
  function build(def) {
    ...
    /* PLANET SAG. The board is now a biome on a globe: every height is pulled
       ...
       and the world surface below catches the edges. Gameplay stays flat
       enough: units and click targets use heightAt, which knows the sag. */
    ...
    /* SHORELINE TUCK. The boundary ring dips below the world's surface (the
       world is the same parabola minus an underhang). The world then covers
       the last sliver of board from above, so the plate's edge disappears
       under the globe instead of floating on it: no ledge, no detached-rim
       illusion on the steep far side of a small planet. */
    for (var sgj = 0; sgj < HH; sgj++) {
      for (var sgi2 = 0; sgi2 < HW; sgi2++) {
        var isEdge = (sgi2 === 0 || sgj === 0 || sgi2 === HW - 1 || sgj === HH - 1);
        if (isEdge) height[sgj * HW + sgi2] -= 0.55;
      }
    }
    ...
    for (j = 0; j < GH; j++) {
      for (i = 0; i < GW; i++) {
        ...
        var core = onPath && pd < pathWidth * 0.55;
    ...
    /* A REAL GLOBE. The world is now a closed ball whose crown is the
       ground's own parabola (the underhang guarantee that killed the old
       poke) and whose far hemisphere is a true sphere lower half. From any
       reachable camera the silhouette is a solid planet with the playfield
       sitting on its crown. */
    var worldR = Math.max(def.planetR || 0, 2 * SAG_R);
    var yOff = -0.30;
    function worldY(r) { return -(r * r) / (2.0 * SAG_R) + yOff; }
    var worldB = MESH.builder('world:' + seed);
    var wSides = 30;
    /* Crown: parabola from the pole to the equator (y decreases with r, the
       same convention the old bowl used and rendered correctly). */
    function crownSpan(t0, t1, col) {
      var prof = [];
      for (var wi = 0; wi <= 6; wi++) {
        var r = (t0 + (t1 - t0) * wi / 6) * worldR;
        prof.push([r, worldY(r)]);
      }
      worldB.color(col);
      worldB.lathe(prof, wSides, wi * 0.35);
    }
    crownSpan(0.001, 0.50, '#241b52');
    crownSpan(0.50, 0.85, '#1a1240');
    crownSpan(0.85, 1.00, '#100b26');
    /* Far hemisphere: bottom pole up to the equator (y INCREASES with the
       profile so the lathe winding stays outward, matching the crown). */
    function farSpan(a0, a1, col) {
      var prof = [];
      for (var wi = 0; wi <= 6; wi++) {
        var a = (a0 + (a1 - a0) * wi / 6) * Math.PI / 2;
        var r = Math.max(0.001, Math.cos(a) * worldR);
        var y = -worldR - Math.sin(a) * worldR + yOff;
        prof.push([r, y]);
      }
      worldB.color(col);
      worldB.lathe(prof, wSides, wi * 0.37);
    }
    farSpan(1.0, 0.55, '#0d0920');
    farSpan(0.55, 0.22, '#0a0718');
    farSpan(0.22, 0.0, '#080512');
    ...
    (function () {
      ...
      for (var vi = 0; vi < Vd.length; vi += 12) {
        ...
        var m = U.smoothstep(boardR, boardR * 1.25, r) * (1.0 - U.smoothstep(worldR * 0.88, worldR * 0.99, r));
        if (m > 0.001) {
          var nl = wnoise(px, pz);
          Vd[vi + 1] = py + nl * 2.8 * m;
    ...
    var worldRocksData = null;
    {
      ...
      for (var wr = 0; wr < 26; wr++) {
        ...
        var rr = Math.max(halfW, halfH) * (1.04 + wrRng() * 0.26);
        if (rr > worldR * 0.86) continue;
