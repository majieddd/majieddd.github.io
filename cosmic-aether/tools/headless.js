...
/* AEGIS_GPU trades the deterministic software rasteriser for the real GPU so
   MSAA renderbuffers and the angular resolve path actually run. The software
   path never exercises them (MSAA is skipped there by design), so a
   renderbufferStorageMultisample bug would pass every CI run until a human
   played the game. */
if (process.env.AEGIS_GPU) {
  const i = chromeFlags.indexOf('--disable-gpu');
  if (i >= 0) chromeFlags.splice(i, 1);
  chromeFlags.push('--use-angle=d3d11', '--enable-gpu');
}
