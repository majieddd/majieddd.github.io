// ===== sim.js : deterministic game logic. No THREE / no DOM. =====
// Attaches GameSim to globalThis so it can be unit-tested under Node (with data.js loaded first).
...
GameSim.prototype.reset = function (seed) {
  this.seed = seed >>> 0;
...
GameSim.prototype.emit = function (e) { this.events.push(e); };

GameSim.prototype.canBuildAt = function (c, r) {
  if (c < 0 || r < 0 || c >= GRID.cols || r >= GRID.rows) return false;
  if (PATH_TILES.has(c + ',' + r)) return false;
  const [cc, cr] = CORE_TILE;
  if (Math.abs(c - cc) <= 1 && Math.abs(r - cr) <= 1) return false;
  for (const t of this.towers) if (t.c === c && t.r === r) return false;
  return true;
};

GameSim.prototype.placeTower = function (type, c, r) {
  ...
  this.gold -= def.cost;
  const t = {
    id: this.towerId++, type, def, c, r, x: c + 0.5, y: r + 0.5,
...
GameSim.prototype.sellTower = function (id) {
...
GameSim.prototype.upgradeTower = function (id) {
...
GameSim.prototype.startNextWave = function (early) {
...
GameSim.prototype._spawn = function (type) {
  const def = ENEMIES[type];
  const p0 = this.path[0];
  const e = {
    id: nextEnemyId(), type, def, hp: def.hp, maxHp: def.hp,
...
GameSim.prototype._applyDamage = function (target, dmg, slow, srcHue) {
...
GameSim.prototype._kill = function (e) {
  if (e.dead) return;
  e.dead = true;
  this.gold += e.reward;
...
GameSim.prototype.update = function (dt) {
...
GameSim.prototype._acquire = function (t) {
...
GameSim.prototype._fire = function (t, target) {
  t.fireAnim = 1;
  const def = t.def;
  this.emit({ type: 'fire', x: t.x, y: t.y, hue: def.color, kind: def.proj, tx: target.x, ty: target.y });
  if (def.proj === 'arc') { this._chainHit(t, target); return; }
  const speed = def.proj === 'beam' ? 38 : def.proj === 'flak' ? 18 : def.proj === 'frost' ? 14 : 22;
  this.projectiles.push({
    id: this.towerId++, x: t.x, y: t.y, targetId: target.id, tx: target.x, ty: target.y,
    speed, dmg: t.dmg, splash: t.splash, slow: t.slow, kind: def.proj, hue: def.color,
    pierce: t.pierce, done: false, life: 2.5,
  });
};

GameSim.prototype._chainHit = function (t, first) {
...
GameSim.prototype._stepProjectile = function (p, dt) {
  p.life -= dt;
  ...
  if (d <= step || d < 0.2) { p.x = tx; p.y = ty; this._resolveProjectile(p, tgt); }
  else { p.x += (dx / d) * step; p.y += (dy / d) * step; }
};

GameSim.prototype._resolveProjectile = function (p, tgt) {
  if (p.splash > 0) {
...
GameSim.prototype._victory = function () { this.state = 'victory'; this.emit({ type: 'victory' }); };
GameSim.prototype._defeat = function () { this.state = 'defeat'; this.emit({ type: 'defeat' }); };
GameSim.prototype.drainEvents = function () { const e = this.events; this.events = []; return e; };

if (typeof globalThis !== 'undefined') globalThis.GameSim = GameSim;
if (typeof module !== 'undefined' && module.exports) module.exports = { GameSim, makeRNG };
